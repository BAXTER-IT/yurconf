...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:conf="http://baxter-it.com/config"
    exclude-result-prefixes="xs conf" version="2.0">
    ...
    <xsl:template match="conf:reference" mode="url">
        ...
        <xsl:value-of select="/request/@base"/>
        <xsl:text>/</xsl:text>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$configurationVariant">
