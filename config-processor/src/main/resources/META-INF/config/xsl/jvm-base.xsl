...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="build-jvm-options">
		<xsl:apply-templates
			select="j:debug/c:component[@id=$configurationComponentId]/j:port" />
		<xsl:apply-templates
			select="j:logging[c:component[@id=$configurationComponentId]]" />
		<xsl:apply-templates
			select="j:option[c:component[@id=$configurationComponentId]]" />
		<xsl:apply-templates
			select="j:heap[c:component[@id=$configurationComponentId]]" />
		<xsl:apply-templates
			select="j:gc[c:component[@id=$configurationComponentId]]" />
		<xsl:apply-templates
		...
		<xsl:if test="/conf:request/conf:variant">
			<xsl:apply-templates select="/j:configuration"
				...
				<xsl:with-param name="opt">
					...
					<xsl:value-of select="/conf:request/conf:variant/@id"
						separator="," />
				</xsl:with-param>
			</xsl:apply-templates>
		</xsl:if>
	...
	<xsl:template
		match="j:system[@var][/conf:request/conf:parameter[@id='osfamily']/text()='unix']">
		<xsl:apply-templates select="/j:configuration"
			mode="append-opt">
			<xsl:with-param name="opt">
				<xsl:text>-D</xsl:text>
