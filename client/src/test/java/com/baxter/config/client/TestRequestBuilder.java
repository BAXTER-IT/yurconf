...
class TestRequestBuilder
{
  ...
  @Test
  void buildWithType() throws Exception
  {
  ...
  @Test
  void buildWithType2() throws Exception
  {
	final Environment env = mock(Environment.class);
	when(env.getProductId()).thenReturn("dummy.id");
	when(env.getComponentId()).thenReturn("mocked");
  ...
  @Test
  void requestWithParameter() throws Exception
  {
	final Environment env = mock(Environment.class);
	when(env.getProductId()).thenReturn("dummy.id");
	when(env.getComponentId()).thenReturn("mocked");
  ...
  @Test
  void requestWithParameters() throws Exception
  {
	final Environment env = mock(Environment.class);
	when(env.getProductId()).thenReturn("dummy.id");
	when(env.getComponentId()).thenReturn("mocked");
	...
	final RequestBuilder b = new RequestBuilder().forType("asis").addParameter("file", "constants.license").addParameter("local",
	    "yes");
