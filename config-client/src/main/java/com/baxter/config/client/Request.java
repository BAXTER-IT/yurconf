...
public class Request
{
  ...
  Request(final URL restUrl, final ConfigID configId, final Version version,
      final List<ConfigParameter> parameters) throws MalformedURLException
  {
    final ConfigParameter[] params = parameters
        .toArray(new ConfigParameter[parameters.size()]);
    url = buildRequestURL(restUrl, configId, version, params);
  }

  private static URL buildRequestURL(final URL restUrl,
      final ConfigID configId, final Version version,
      ...
      for (int i = 0; i < parameters.length; i++)
      {
        ...
        catch (final UnsupportedEncodingException e)
        {
          throw new IllegalStateException(
              "Cannot encode parameter value for URL", e);
        }
      }
    }
  ...
  public <T> T loadObject(final Class<T> objectClass)
